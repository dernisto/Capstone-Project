  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50 dark:from-slate-950 dark:via-indigo-950 dark:to-purple-950 flex items-center justify-center p-4">
      <div className="glass-panel p-8 max-w-md w-full text-center">
        ...
        {pin ? (
          <>
            <div className="text-4xl font-mono font-bold tracking-widest mb-6 bg-slate-100 dark:bg-slate-800 py-4 px-6 rounded-xl">{pin}</div>
            <div className="flex flex-col sm:flex-row gap-3 justify-center mb-6">
              <Button onClick={copyPin} variant="outline" size="lg" className="gap-2">
                {copied ? <Check className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                {copied ? "Copied!" : "Copy PIN"}
              </Button>
              <Button onClick={() => window.open(joinUrl, "_blank")} className="btn-primary gap-2">
                <ExternalLink className="w-5 h-5" /> Open Join page
              </Button>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Or share this link so the PIN is pre-filled:</p>
            <p className="text-sm font-mono text-indigo-600 dark:text-indigo-400 break-all mb-8">{joinUrl}</p>
          </>
        ) : (
